package com.alexjck.plugins.scheduled.conditional.alerts;
...
public class ScheduledConditionalCountAlertCondition extends AbstractAlertCondition {
	private static final Logger LOG = LoggerFactory.getLogger(ScheduledConditionalCountAlertCondition.class);

	enum ThresholdType {

		MORE("more than"), LESS("less than");
	...
	public interface Factory extends AlertCondition.Factory {
		@Override
		ScheduledConditionalCountAlertCondition create(Stream stream, @Assisted("id") String id, DateTime createdAt,
	...
	public static class Config implements AlertCondition.Config {
		...
		public ConfigurationRequest getRequestedConfiguration() {
			final ConfigurationRequest configurationRequest = ConfigurationRequest.createWithFields(
					new TextField("query", "Query", "", "Query that should be checked",
							ConfigurationField.Optional.NOT_OPTIONAL),
					new NumberField("time", "Time Range", 5,
							"Evaluate the condition for all messages received in the given number of minutes",
							ConfigurationField.Optional.NOT_OPTIONAL),
					new TextField("crontabExpression", "Crontab", "0 * * * * *",
							"Schedule this conditional call with a crontab",
							ConfigurationField.Optional.OPTIONAL),
					new DropdownField("threshold_type", "Threshold Type", ThresholdType.MORE.toString(),
							Arrays.stream(ThresholdType.values())
									.collect(Collectors.toMap(Enum::toString, ThresholdType::getDescription)),
							"Select condition to trigger alert: when there are more or less messages than the threshold",
							ConfigurationField.Optional.NOT_OPTIONAL),
					new NumberField("threshold", "Threshold", 0.0, "Value which triggers an alert if crossed",
							ConfigurationField.Optional.NOT_OPTIONAL));
	...
	@AssistedInject
	public ScheduledConditionalCountAlertCondition(Searches searches, Configuration configuration,
			@Assisted Stream stream, @Nullable @Assisted("id") String id, @Assisted DateTime createdAt,
			@Assisted("userid") String creatorUserId, @Assisted Map<String, Object> parameters,
			@Assisted("title") @Nullable String title) {
		super(stream, id, ScheduledConditionalCountAlertCondition.class.getCanonicalName(), createdAt, creatorUserId,
				parameters, title);
		this.searches = searches;
		this.configuration = configuration;
		this.query = (String) parameters.get("query");
		this.time = Tools.getNumber(parameters.get("time"), 5).intValue();
		this.crontabExpression = (String) parameters.get("crontabExpression");
		...
		try {
			crontab = new CronExpression(crontabExpression);
			this.nextFireTime = crontab.getNextValidTimeAfter(cal.getTime()).getTime();
	...
	public CheckResult runCheck() {
		try {
			...
			if (triggered) {
				...
				if (getBacklog() > 0) {
					final SearchResult backlogResult = searches.search(query, filter, range, getBacklog(), 0,
		...
		} catch (InvalidRangeParametersException | ParseException e) {
	...
	@Override
	public String getDescription() {
		return "time: " + time + ", threshold_type: " + thresholdType.toString().toLowerCase(Locale.ENGLISH)
				+ ", threshold: " + threshold + ", grace: " + grace + ", query: " + query + ", repeat notifications: "
				+ repeatNotifications;
	}
}